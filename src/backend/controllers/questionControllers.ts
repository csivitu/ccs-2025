...
export const submitQuestion = async (req: Request, res: Response) => {
  const { questionId, ccsUserId, answer } = req.body;
  try {
    const questionAttempt = await prisma.questionAttempt.create({
      data: {
        questionId,
        ccsUserId,//need to switch to db call probly a fn in user controller like prev repo
        answer,
      },
    });
    return res.status(201).json(questionAttempt);
  } catch (error: any) {
    return res.status(500).json({ error: error.message });
  }
};
...
export const submitTask = async (req: Request, res: Response) => {
  const {  ccsUserId, task, subdomain } = req.body;
  try {
    ...
    return res.status(201).json(taskSubmission);
  } catch (error: any) {
    return res.status(500).json({ error: error.message });
  }
};
